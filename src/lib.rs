...
//! - Rustc feature `associated_type_defaults` simplifies handling default impls **recommended**
//! 
//! Cons:
//! - Tracking versions can get verbose
//! - Implementing types and functions for specific versions requires manual selection over each
//!   version, until the latest which can still use the Since<L> selector.
//!   - This could use the `Range` syntax only if rustc permits `Until<R>: !Since<R>`, which would
//!     convince rustc that the two sets never intersect.
//!
//! Todo:
...
//  impl<R: Range<Alice, Bob>> FooTys for R {}
    ...
    trait FooBob<R> where R: FooTys {
        ...
        /// Type checks while the runtime only calls bob() when `can_bob` returns true
    ...
    // Can use Range<Since, Until> when implementing against types, but not for traits
    ...
    fn dependant_versions() {
        ...
        assert_eq!(f::<Alice>(), None);     // Alice does not manually implement bob
        assert_eq!(f::<Bob>(), Some(true)); // Bob does manually implement bob
    }
}
